    ...
    setInterval(async () => {
        ...
        timers.forEach(async (serverTimer) => {
            ...
            if (new Date(serverTimer.ends_at) <= new Date().getTime()) {
                await channel
                    ...
                    .then(() => {
                        message
                            .edit({
                                content: global.t.trans(['info.fun.timer.timerEnded'], channel.guild.id),
                            })
                            .catch((err) => {});
                    })
                    .catch((err) => {
            ...
            if (minutes <= 0 && hours <= 0 && days <= 0) {
                ...
                timeLeftString = global.t.trans(['info.fun.timer.timeLeftTime', days, hours, minutes], channel.guild.id)
            }
