...
module.exports.getFormByGuild = async ({
    guild_id
}) => {
...
module.exports.getFormById = async ({
    guild_id,
    apply_id
}) => {
...
module.exports.sendApplyForm = async ({
    apply_id,
    main_interaction,
}) => {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        const channel = main_interaction.bot.guilds.cache.get(main_interaction.guild.id).channels.cache.get(applyForm.channel);
...
module.exports.saveApplyForm = async ({
    ...
    return new Promise(async (resolve, reject) => {
        ...
        await database.query(`UPDATE guild_config SET apply_form = ? WHERE guild_id = ?`, [JSON.stringify(res), guild_id])
            .then(() => {
...
module.exports.manageNewForm = async ({
    main_interaction,
}) => {
    return new Promise(async (resolve, reject) => {
        ...
        const sentMessage = await main_interaction.channel.send(`🔎 Please add the content for your choosen setting. ||\`cancel\` stops the current collector, \`clear\` will clear the current option to ''|| ${(value.search('field') !== -1) ? `\n\n**Note:** If you want to add a field, you have to use the following format (With the **:**!!): \n\n\`yourfield_name:yourfield_value\`` : ''}`)
        ...
        collector.on('collect', async (message) => {
            ...
            switch (value) {
                ...
                case 'thumbnail':
                    ...
                    if (validURL(data[value]) && data[value].endsWith('jpg') || data[value].endsWith('png')) {
...
module.exports.manageApplication = async ({
    main_interaction,
    apply_id
}) => {
    var applyForm = await this.getFormById({
    ...
    const channel_exists = main_interaction.guild.channels.cache.find(channel => channel.name.toLowerCase() === channel_name);
    ...
    if (!category) {
        return main_interaction.reply({
            content: '❌ The administration didnt passed any Category. Please contact the server moderation team.',
            ephemeral: true
        }).catch(err => {})
    } else {
        main_interaction.guild.channels.create(`application-${main_interaction.user.username}-${apply_id}`, {
            type: 'text',
            parent: category,
            position: 1,
