...
module.exports.getMemberInfo = async () => {
    return await database.query(`SELECT * FROM member_info`)
        .then(res => {
            if (res.length > 0) {
                return res;
            } else {
...
module.exports.getMemberInfoById = async ({
    guild_id,
    user_id
}) => {
    ...
    if (cache) {
        var userInfo = cache[0].memberInfo.find(x => x.user_id === user_id && x.guild_id === guild_id);
        return userInfo || false;
    }
    return await database.query(`SELECT * FROM member_info WHERE user_id = ? AND guild_id = ?`, [user_id, guild_id])
        .then(res => {
            if (res.length > 0) {
                return res[0];
            } else {
...
module.exports.updateMemberInfoById = async ({
    guild_id,
    user_id,
    member_roles,
    user_joined
