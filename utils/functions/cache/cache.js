...
module.exports.updateCache = async ({
    ...
    for (let i in this[cacheName]) {
        if (this[cacheName][i].id === param_id[0]) {
            // If both id's match
            ...
            if (typeof updateVal === 'object' && updateVal !== undefined) {
                for (const [index, [key, value]] of Object.entries(Object.entries(updateVal))) {
                    ...
                    if (value === updateVal.role_id) {
                        // hardcode for modroles
                        this[cacheName][i] = updateVal;
                    }
                }
            } else {
                // updateval is a string or undefined
                if (updateVal === undefined) {
                    // updateval is undefined
                    if (updateValName === 'roles' || updateValName === 'role_id') {
                        // hardcode for warnroles
                        ...
                        this[cacheName][i][updateValName] =
                            this[cacheName][i][updateValName].filter(Boolean);
                    }
                } else {
                    // updateval is a string
                    if (typeof this[cacheName][i][updateValName] === 'array') {
                        this[cacheName][i][updateValName].push(updateVal);
                    } else if (cacheName === 'xp') {
                        this[cacheName][i][updateValName].find(
                            (x) => x.user_id === param_id[1]
                        ).xp = updateVal;
                    } else {
