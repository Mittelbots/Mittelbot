...
module.exports.viewAllSettings = async ({currentsettings, message, bot}) => {
    ...
    for (let i in config.settings) {
        ...
        if (config.settings[i].colname === config.settings.wc.colname){ //? Wenn Settings gleich Welcome channel
            ...
            current = await database.query(`SELECT role_id FROM ${message.guild.id}_guild_joinroles`).then(res => {
                if (res.length > 0) {
                    current = '';
                    for (let x in res) {
            ...
            current = await database.query(`SELECT auditlog, messagelog, modlog FROM ${message.guild.id}_guild_logs`).then(res => {
                if (res.length > 0) {
            ...
            current = await database.query(`SELECT * FROM ${message.guild.id}_guild_warnroles`).then(res => {
                if (res.length > 0) {
                    current = '';
                    for (let x in res) {
                        current += `<@&${res[x].role_id}> `;
