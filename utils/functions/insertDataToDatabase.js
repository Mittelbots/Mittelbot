...
function insertDataToOpenInfraction (uid, modid, mute, ban, till_date, reason, infraction_id) {
    database.query('INSERT INTO open_infractions (user_id, mod_id, mute, ban, till_date, reason, infraction_id) VALUES (?, ?, ?, ?, ?, ?, ?)',[uid, modid, mute, ban, till_date, reason, infraction_id]).catch(err => console.log(err));
}

function inserDataToTemproles (uid, role_id, till_date, infraction_id) {
    database.query('INSERT INTO temproles (user_id, role_id, till_date, infraction_id) VALUES (?, ?, ?, ?)', [uid, role_id, till_date, infraction_id]).catch(err => console.log(err));
}
