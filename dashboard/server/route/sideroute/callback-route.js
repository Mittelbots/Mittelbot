        ...
        if (code) {
            ...
            if (user) {
                ...
                if (userExists.length === 0) {
                    await database
                        .query(
                            `INSERT IGNORE INTO dashboard_users (user_id, username, email, verified, loginToken) VALUES (?, ?, ?, ?, ?);`,
                            [
                                user.data.id,
                                user.data.username,
                                user.data.email,
                        ...
                        .catch((err) => {
                            ...
                            return res.redirect(
                                app.settings.config.route.homepage.path +
                                    '?error=true&message=Something went wrong, please try again.'
                            );
                        });
                } else {
                    await database
                        .query(`UPDATE dashboard_users SET loginToken = ? WHERE email = ?`, [
                            token,
                            user.data.email,
                        ])
                        .catch((err) => {
